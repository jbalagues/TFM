...
public class CargaAparcamientoBici   extends CargaDatos{
	static final String agencia = "PARKING_BICI";
	
	public CargaAparcamientoBici(){
		...
		super(Logger.getLogger(CargaAparcamientoBici.class));
	...
	public static void main(String[] args) {		
		...
		String ciudad = "BCN";
		String properties = "aparcamientoBicis.properties";
		...
		CargaAparcamientoBici pt= new CargaAparcamientoBici();
	...
	public void execute(String ciudad, String properties){
		logger.info("Inicio carga de aparcamientos de bicis");
		...
		logger.info("Fin carga de aparcamientos de bicis");
	}
	/*
	...
	public List<TransporteBean> readXML2(String ciudad, String path, String file) throws ParserConfigurationException, SAXException, IOException, XPathExpressionException{
		...
		for (int i = 0; i < nodes.getLength(); i++) {
		    ...
		    if(nodesRow.getLength()>0){
		    	...
		    	ptBean.setAgency("ANCORAJE_BICI");	
		    	...
		    	ptBean.setPuntuacion(Constantes.PUNTUACION_ANCORAJE_BICI);
		    	for (int j = 0; j < nodesRow.getLength(); j++) {
		    		if(nodesRow.item(j) instanceof Element){
		    			...
		    			String value = eItem.getFirstChild().getNodeValue(); //nodesValues.item(0).getNodeValue();
